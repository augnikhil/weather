...
const Sun = ({ width, height }) => {
  const { myweatherdata, convertHMS,white } = useWeatherContext();
  var spin1 = keyframes``;
  ...
  var spin = keyframes`
          ...
          100% { transform: rotate(${
            ((myweatherdata.current.dt - myweatherdata.daily[0].sunrise) / 60) *
              (180 /
                ((myweatherdata.daily[0].sunset -
                  myweatherdata.daily[0].sunrise) /
                  60)) -
            90
          }deg); }
      `;
  ...
  const MyRotate = styled.div`
    transform: rotate(
      ${((myweatherdata.current.dt - myweatherdata.daily[0].sunrise) / 60) *
        (180 /
          ((myweatherdata.daily[0].sunset - myweatherdata.daily[0].sunrise) /
            60)) -
      90}deg
    );
      ...
      >
        ...
        <MyRotate>
          ...
          >
            ...
            >
              {convertHMS(myweatherdata.current.dt, "minutes")}
            </div>
          </div>
        </MyRotate>
        <div
        ...
        >
          ...
          >
            ...
            <span>{convertHMS(myweatherdata.daily[0].sunrise, "minutes")}</span>
          </div>
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            <span>{convertHMS(myweatherdata.daily[0].sunset, "minutes")}</span>
          </div>
        </div>
      </div>
    </div>
